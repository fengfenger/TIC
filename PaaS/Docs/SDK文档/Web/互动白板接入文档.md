...
移动端支持mp4/m3u8，桌面端支持mp4/m3u8/flv/rtmp）；触发状态改变回调 TEB_VIDEO_STATUS_CHANGED
...
是否同步本地视频操作状态到远端，play接口/pause接口/seek接口以及控制栏事件触发是否影响远端，默认为true，全局控制项，对所有视频文件有效
...
内部启动定时器，定时同步视频状态到远端（仅限于mp4），一般在老师端视频加载完成后调用，切换文件后自动销毁定时器，只对当前文件有效
