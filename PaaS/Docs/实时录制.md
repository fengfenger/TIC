...
| video_type      | int    | 视频流类型，0：摄像头视频，2：白板视频，3：混流视频，4：纯音频文件     |
...
| audio_file_needed | bool | 是否需要在结果回调中返回各路流的纯音频录制文件，文件格式为mp3 | 否 |
...
| init_param | string | 白板初始化参数，透传到白板 SDK，参数详见 [互动白板接入文档](./SDK文档/互动白板接口文档.md) | 否      |
...
| model_id | int | 内置混流布局模板 ID，取值 [1, 2]，区别见内置混流布局模板样式示例说明 | 否, 未填写 custom 字段时，此字段必填|
| teacher_id | string | 老师用户 ID <br> 填写 teacher_id 是把指定为 teacher_id 的用户视频流显示在内置模板的第一个小画面中 | 否，此字段只在 model_id 已填写时生效 |
| custom | CustomLayout | 自定义混流布局参数 | 否，当此字段存在时，model_id 及 teacher_id 字段将被忽略 |
...
| 参数名 | 类型 | 描述 | 是否必填 |
| :--- | :--- | :--- | :--- |
| layout_params.width | int | 混流画布宽，取值范围[2,3000] | 是 |
| layout_params.height | int | 混流画布高，取值范围[2,3000] | 是 |
...
| 参数名 | 类型 | 描述 | 是否必填 |
| :--- | :--- | :--- | :--- |
| layout_params.width | int | 流画面宽，取值范围[2,3000] | 是 |
| layout_params.height | int | 流画面高，取值范围[2,3000] | 是 |
