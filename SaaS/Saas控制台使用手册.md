...
1. 基础信息
   ...
   | 填写信息 | 说明 |
   | --- | --- |
   | 机构 logo | 用户可填写机构 logo 图片的 URL 地址。logo 图片大小要求为100px * 100px，类型为 png/jpg。URL 地址更换后，桌面端上课页面左上角 logo 更换。|
...
- 高级选项
   ...
   | 选项 | 说明 |
   | --- | --- |
   | 助教 | 选择该节课的助教（非必选）。助教协助老师进行上课，拥有老师角色的几乎所有权限。|
   | 学生 | 允许进入该节课的学生列表，若不填写则登录课堂的用户都可进入。单击【学生】，在弹框列表中搜索或点击选择允许进入课堂的学生，学生 ID 将自动填写在课堂表单中。可选择多个学生或通过班级批量选择学生，也可在弹框列表单击【添加学生】，创建学生后再进行选择（操作同添加用户）。|
   | 进房人数限制 | 每个在线课堂可容纳的学生人数，默认10000人，超过限制时，新的人员无法进入课堂。用户可自行设置。|
