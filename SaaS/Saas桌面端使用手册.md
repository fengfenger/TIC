...
要素|说明
:--|:--
机构编号|由管理员告知。腾讯云互动课堂分配的公司编号。
课堂编号|由管理员告知。约好的课程编号。
帐号|由管理员告知。登录 App 的账号，仅管理员可修改。
密码|由管理员告知。登录 App 的密码，仅管理员可修改。


 其中，【机构编号】 只需要第一次使用时输入，表示绑定哪个教育机构，之后再次打开 App，会默认使用第一次输入的值（支持手动修改）。**卸载 App 后不保留默认值。**
然后点击加入课堂即可进入【课堂编号】对应的课堂中，如果某一个要素输入错误，则会弹出提示框，请根据提示信息进行操作。
